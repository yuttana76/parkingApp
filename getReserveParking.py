...
def get_reserve_parking_json():
    try:
        ...
        if Parking.qr_code_reserve != None and Parking.qr_code_gateout_status == "1" :
            status_qr = 0
        elif Parking.qr_code_reserve == None and Parking.payment_status != "1": #กรณียืนยันการจองแล้วแต่ยังไมจ่าย
            status_qr = 0
...
def reserve_period_price(station):
    ...
    stations['img'] = columns.parking_image
    stations['location'] = columns.location
    stations['office_time'] = columns.office_time
    stations['checkin_period_hr'] = round(int(columns.reserve_checkin_period)/60)
    return stations
...
def insert_reserve_log(station, floor, form):
    reserve_log = {}
    ...
    orderNumber = gen_reserve_ref2(id_,reserve_log['no'])
    reserve_log['orderNumber'] = orderNumber
    reserve_log['station'] = station
    reserve_log['reserve_floor'] = floor
    reserve_log['address_type'] = '1'

    # tax invoice home
    reserve_log['unit_home'] = form.unit_home.data
    reserve_log['address_home'] = form.address_home.data
    reserve_log['village_home'] = form.village_home.data
    reserve_log['alley_home'] = form.alley_home.data
    reserve_log['road_home'] = form.road_home.data
    reserve_log['district_home'] = form.district_home.data
    reserve_log['sub_district_home'] = form.sub_district_home.data
    reserve_log['postal_code_home'] = form.postal_code_home.data
    reserve_log['province_home'] = form.province_home.data

    # tax invoice company
    reserve_log['company_unit'] = form.unit_company.data 
    reserve_log['identity_com'] = form.identity_com.data
    reserve_log['company_name'] = form.company_name.data
    reserve_log['company_address'] = form.address_company.data
    reserve_log['company_village'] = form.village_company.data
    reserve_log['company_alley'] = form.alley_company.data
    reserve_log['company_street'] = form.road_company.data
    reserve_log['company_province'] = form.province_company.data
    reserve_log['company_district'] = form.district_company.data
    reserve_log['company_sub_district'] = form.sub_district_company.data
    reserve_log['company_postal_code'] = form.postal_code_company.data

    #Agreement
    reserve_log['accept_agreement'] = '1'
    ...
    new_log = Parking_log(
        ...
        # tax invoice home
        unit_home = reserve_log['unit_home'],
        address_no = reserve_log['address_home'],
        village = reserve_log['village_home'],
        alley = reserve_log['alley_home'],
        street = reserve_log['road_home'],
        district = reserve_log['district_home'],
        sub_district = reserve_log['sub_district_home'],
        postal_code = reserve_log['postal_code_home'],
        province = reserve_log['province_home'],

        # tax invoice company
        company_unit = reserve_log['company_unit'], 
        identity_com = reserve_log['identity_com'],
        company_name = reserve_log['company_name'],
        company_no = reserve_log['company_address'],
        company_village = reserve_log['company_village'],
        company_alley = reserve_log['company_alley'],
        company_street = reserve_log['company_street'],
        company_province = reserve_log['company_province'],
        company_district = reserve_log['company_district'],
        company_sub_district = reserve_log['company_sub_district'],
        company_postal_code = reserve_log['company_postal_code'],

        accept_agreement = reserve_log['accept_agreement'],
        address_type = reserve_log['address_type']
    )
...
def gen_qr_parking_log():
    ...
    available_parking_code = ['N1013', 'N1014', 'BL150']
    # print(parking_code_cit)
    id_ = Parking_log.query.filter(Parking_log.Id >= 0).order_by(Parking_log.Id.desc()).first().Id+1
    ref1 = "3333333333333"
    ref2 = gen_reserve_ref2(id_,parking_code_cit)
    # print(ref2)
    if request.method == 'POST' and username == 'jparkapi' and password == '123456' :
        
        if parking_code_cit in available_parking_code:
            code = '100' 
            parking = Parking_manage.query.filter_by(parking_code = parking_code_cit).first()
            parking_name = parking.parking_name_eng
...
def gen_reserve_ref2(id_,stationCode):
    zero_filled_number = str(id_).zfill(10)
    ref2 = "REV"+stationCode+zero_filled_number
    return ref2
